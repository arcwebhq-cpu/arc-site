...
- The endpoint requires Stripe API version `2026-08-26.dahlia`, the configured test/live mode, the expected ARC account hash, and a pre-registered Checkout Session → PaymentIntent → handoff binding.
...
2. Pin that destination to the stable API version `2026-08-26.dahlia` (never
   the `2026-07-29.preview` channel) and select exactly
   this complete union:
