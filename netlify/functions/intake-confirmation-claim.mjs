...
export function createIntakeConfirmationClaimHandler() {
  return async (request, context = {}) => {
    ...
    try {
      ...
      return json(200, await claimIntakeConfirmationOutbox(raw, request, process.env, store, { clock: context.clock }));
    } catch (error) {
      if (error?.message === 'ARC_INTAKE_CONFIRMATION_UNAUTHORIZED') return json(401, { error: 'unauthorized' });
      if (error?.message === 'ARC_INTAKE_CONFIRMATION_NOT_FOUND') return json(404, { error: 'confirmation_not_found' });
      if (/ARC_INTAKE_CONFIRMATION_(?:CLAIM_CONFLICT|REVIEW_REQUIRED|TERMINAL)/.test(error?.message || '')) {
...
const handler = createIntakeConfirmationClaimHandler();

export default createRetentionFencedRouteHandler({
  route: 'intake-confirmation-claim',
  paths: ['/internal/intake/confirmation/claim'],
...
export const config = {
  path: '/internal/intake/confirmation/claim', method: 'POST',
